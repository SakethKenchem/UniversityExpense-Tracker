...
<head>
  ...
  <style>
    ...
    /* Scrollable expense table body */
    .expense-table-wrapper {
      max-height: 220px; /* Adjust for 5 rows */
      overflow-y: auto;
      width: 100%;
    }
...
<body>
  <div class="container">
    ...
    <div class="row g-3 mb-4">
      ...
      <div class="col-md-4">
        <div class="card p-3">
          ...
          <button class="btn btn-secondary w-100" onclick="exportCSV()">Export CSV (DB)</button>
        </div>
      </div>
    </div>
    ...
    <div class="row mb-4">
      <div class="col-md-4">
        <select id="monthFilter" class="form-control" onchange="fetchAndRender()">
  ...
  <script>
    // Fetch from DB and render everything
    async function fetchAndRender() {
      ...
      renderTables(data.expenses, data.incomes);
      renderSummary(data.expenses, data.incomes);
      populateMonths(data.expenses);
      drawChart(data.incomes);
      drawExpenseChart(data.expenses); // Draw expense pie chart
    }
    ...
    async function addExpense() {
      ...
      if (result.success) {
        clearInputs(['amount', 'description', 'date']);
        fetchAndRender();
      } else {
    ...
    async function addIncome() {
      ...
      const result = await res.json();
      if (result.success) {
        clearInputs(['incomeAmount', 'incomeDesc', 'incomeDate']);
        fetchAndRender();
      } else {
    ...
    function renderTables(expenses, incomes) {
      ...
      incomes.forEach(i => {
        incomeTable.innerHTML += `<tr>
          ...
          <td><button class="btn btn-sm btn-danger" onclick="deleteIncome(${i.id})">Delete</button></td>
        </tr>`;
      });
    }

    function renderSummary(expenses, incomes) {
    ...
    function populateMonths(expenses) {
      const select = document.getElementById('monthFilter');
      // Get unique months sorted descending
      const uniqueMonthsSet = new Set(expenses.map(e => `${e.year}-${String(e.month).padStart(2, '0')}`));
      const uniqueMonths = Array.from(uniqueMonthsSet).sort((a, b) => b.localeCompare(a));
    ...
    let myPie, myExpensePie;

    function drawChart(incomes) {
    ...
    function drawExpenseChart(expenses) {
      ...
      if (myExpensePie) myExpensePie.destroy();
      myExpensePie = new Chart(ctx, {
        type: 'pie',
        data: {
          labels: Object.keys(grouped),
          datasets: [{
    ...
    async function deleteExpense(id) {
      ...
      const res = await fetch('delete_expense.php?id=' + id, {
        method: 'DELETE'
      });
      const result = await res.json();
      if (result.success) fetchAndRender();
      else alert('Failed to delete expense.');
    }

    async function deleteIncome(id) {
      if (!confirm('Delete this income?')) return;
      const res = await fetch('delete_income.php?id=' + id, {
        method: 'DELETE'
      });
      const result = await res.json();
      if (result.success) fetchAndRender();
    ...
    function exportCSV() {
      const month = document.getElementById('monthFilter').value;
      // Open CSV export with month filter param
